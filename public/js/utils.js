...
function saveNewSlate() {
    ...
    if(type === 'text') {
        ...
        compressImage(file).then(compressedBlob => {
            if(compressedBlob.size > 5000000) return alert('Compressed image still too large. Please choose a smaller image.');
...
function compressImage(file, maxWidth = 3840, quality = 0.9) {
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            const targetWidth = isPng ? img.width : Math.min(maxWidth, img.width);
